...
IMAGES_DIR=images
DEBIAN_ISO=debian-11.4.0-amd64.iso
...
setup_debian() {
    if [ ! -f $IMAGES_DIR/$DEBIAN_DIR ]; then
        ...
        if [ ! -f $IMAGES_DIR/$DEBIAN_ISO ]; then
            ...
            wget -q -O $IMAGES_DIR/$DEBIAN_ISO $DEBIAN_URL
        fi
        bsdtar -C $DEBIAN_DIR -xf $IMAGES_DIR/$DEBIAN_ISO
    fi
}
...
run_iso() {
    ...
    -hda $IMAGES_DIR/$DISK_IMAGE \
    -cdrom $IMAGES_DIR/$DEBIAN_ISO  \
    -boot d -enable-kvm
}
